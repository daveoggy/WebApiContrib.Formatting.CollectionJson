    ...
    public class CollectionJsonFormatter : JsonMediaTypeFormatter
    {
        ...
        public override bool CanWriteType(Type type)
        {
            return base.CanWriteType(type) &&
                   (typeof (IReadDocument).IsAssignableFrom(type) || typeof (IWriteDocument).IsAssignableFrom(type));
        }
        ...
        public override System.Threading.Tasks.Task<object> ReadFromStreamAsync(Type type, Stream readStream,
        ...
        public override System.Threading.Tasks.Task WriteToStreamAsync(Type type, object value, Stream writeStream,
            System.Net.Http.HttpContent content, System.Net.TransportContext transportContext)
        {
            if (typeof (IReadDocument).IsAssignableFrom(type) && type != typeof (ReadDocument))
            {
                return base.WriteToStreamAsync(typeof (IReadDocument), new ReadDocumentDecorator((IReadDocument) value),
                    writeStream, content, transportContext);
            }

            if (typeof (IWriteDocument).IsAssignableFrom(type) && type != typeof (WriteDocument))
            {
                return base.WriteToStreamAsync(typeof (IWriteDocument),
                    new WriteDocumentDecorator((IWriteDocument) value), writeStream, content, transportContext);
            }
